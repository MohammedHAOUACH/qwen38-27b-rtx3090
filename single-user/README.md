...
| `VISION` | 0 | 1 keeps the vision tower instead of `--language-model-only` (0.858 GiB of BF16 weights on this checkpoint), for a client that sends images: 32 images per prompt (`MM_COUNT`) and a 1024-image-token pixel cap (`MM_LONGEST`), both overridable |
